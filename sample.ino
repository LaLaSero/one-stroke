...
void loop() {
  ...
  if (currentTime - prevTime >= controlInterval) {
    ...
    // 右側モータを正方向に回転
    Serial.println("右側モータを正方向に回転開始");
    setMotorRight(150); // PWM値を150で回転
    delay(2000); // 2秒間回転
    setMotorRight(0); // モータ停止
    Serial.println("右側モータ停止");

    // 左側モータを反方向に回転
    Serial.println("左側モータを反方向に回転開始");
    setMotorLeft(-150); // PWM値を-150で回転
    delay(2000); // 2秒間回転
    setMotorLeft(0); // モータ停止
    Serial.println("左側モータ停止");
    ...
    // デバッグ用シリアル出力
    Serial.print("右側エンコーダーカウント: "); Serial.print(rightEncoderCount);
    Serial.print(" | 右側角度: "); Serial.print(theta1_current);
    Serial.print(" || 左側エンコーダーカウント: "); Serial.print(leftEncoderCount);
    Serial.print(" | 左側角度: "); Serial.println(theta2_current);
